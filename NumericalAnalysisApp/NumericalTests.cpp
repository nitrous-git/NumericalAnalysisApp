...
void NumericalTests::TestLagrange()
{
	// Polynome de Lagrange 
	// Interpolation de sin(x) avec 3 points
    std::cout << "--------------------------------------------------" << std::endl;
    std::cout << "Polynome de Lagrange" << std::endl;
...
void NumericalTests::TestNewtonGregory()
{
    // Newton Gregory Difference finies (descendantes)
    // Interpolation de sin(x)
    std::cout << "--------------------------------------------------" << std::endl;
    std::cout << "Newton Gregory Difference descendantes" << std::endl;
    ...
    {
        std::pair<double, double> result = NewtonGregory::Interpolate(xData, fData, xEval, degree);
        ...
        std::cout << "  P_" << degree << "(" << xEval << ") = " << std::fixed << std::setprecision(6) << interpolatedValue << std::endl;

        std::cout << "  Erreur reelle = " << std::fixed << std::setprecision(6) << realError << std::endl;

        if (degree < static_cast<int>(xData.size()) - 1)
        {
            std::cout << "  Erreur estimee = " << std::fixed << std::setprecision(6) << estimatedError << std::endl;
        }
        else
        {
            std::cout << "  Erreur estimee = non disponible (pas de terme suivant)" << std::endl;
        }
